...
var Subscriber = (function(){
	...
	function Subscribe_Topic(incident){
		...
		weatherDuration = $sub.find('input')[1].value;
		...
		subscribers_array.forEach(function(sub, pos){
			if(name.toLowerCase() == sub.name.toLowerCase()){
				...
				if(sub.TopicNames.indexOf(TopicName) == -1){
					subscribers_array[pos].Tweets = subscribers_array[pos].Tweets || []
		             subscribers_array[pos].Tweets.push({
			         'TopicName':TopicName,
			           'Details':weatherDuration
		            });
					sub.TopicNames.push(TopicName+" "+weatherDuration);
					_render()
					return;
				}
			}
